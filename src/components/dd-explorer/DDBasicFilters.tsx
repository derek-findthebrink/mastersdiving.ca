...
type DDBasicFiltersProps = Pick<DDFiltersReturn, 'events' | 'boards' | 'setEventSingle' | 'setBoardSingle'>;

const BUTTON_BASE = 'min-h-[44px] px-4 py-2 font-medium text-sm transition-colors border';
const BUTTON_ACTIVE = 'bg-blue-600 text-white border-blue-600 dark:bg-blue-500 dark:border-blue-500';
...
const DDBasicFilters: React.FC<DDBasicFiltersProps> = ({ events, boards, setEventSingle, setBoardSingle }) => {
  ...
  return (
    ...
    >
      <fieldset className="flex flex-wrap items-center gap-2 border-0 p-0">
        <legend className="mb-1 text-sm font-medium text-slate-700 dark:text-slate-300">Event:</legend>
          ...
          {ALL_EVENTS.map((event) => {
            ...
            return (
              <button
                ...
                className={`${BUTTON_BASE} ${isActive ? BUTTON_ACTIVE : BUTTON_INACTIVE} rounded-md`}
      ...
      <fieldset className="flex flex-wrap items-center gap-2 border-0 p-0">
        <legend className="mb-1 text-sm font-medium text-slate-700 dark:text-slate-300">Board:</legend>
        <div className="flex flex-wrap gap-2">
